        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    if (model.UserType.Equals("Customer"))
                    {
...
//                        SaveChanges(_context);
                    ...
                    else
                    {
                        var roleStore = new RoleStore<IdentityRole>(new
                            ApplicationDbContext());
                        var roleManager = new RoleManager<IdentityRole>(roleStore);
                        await UserManager.AddToRoleAsync(user.Id, "Nutritionist");
...
//                        SaveChanges(_context);
        ...
        public JsonResult GetEvents()
        {
            if (User.IsInRole("Customer"))
            {
                ...
                var events = _context.BookingModels.Where(e => e.CustomerId == userId);
                return new JsonResult { Data = events, JsonRequestBehavior = JsonRequestBehavior.AllowGet };
            }
            else
            {
                string userId = User.Identity.GetUserId();
                var events = _context.BookingModels.Where(e => e.NutritionistId == userId);
                return new JsonResult { Data = events, JsonRequestBehavior = JsonRequestBehavior.AllowGet };
            }
...
//            var NutritionistName = _context.Users.ToList();
        ...
        {
            var NutritionistName = _context.Users.Where(x => x.Roles.Select(y => y.RoleId).Contains("2")).ToList();
            bookingViewModel.ApplicationUser = NutritionistName;
            if (ModelState.IsValid)
            {
                ...
                {
                    if (booking.StartTime < bookingViewModel.DateAndTime && booking.EndTime> bookingViewModel.DateAndTime)
                    {
                        ModelState.AddModelError("", "Opps! This nutritionist is not avaliable at this time, Please choose another time.");
                        return View(bookingViewModel);
                    }
                }
